...
import time
# To navigate through sephora pages, change currentPage in url parameter, increase it
# basic sephora url request: https://www.sephora.com/api/v2/catalog/search/?type=keyword&q=foundation&content=true&page=60&currentPage=2&loc=en-CA&ch=rwd&countryCode=CA&targetSearchEngine=nlp
# get product name from imageAltText
class Product:
...
def avoid_product_types(product_name):
    # check if the product name includes any excluded keywords
    excluded_keywords = r"\b(Brush|Collection|Primer|Concealer|Serum|Set|Conditioner|Highlighter|Moisturizer|Gel)\b"
    return not re.search(excluded_keywords, product_name, re.IGNORECASE)
    ...
    if resp.status_code == 200:
        try:
            ...
            for product in current_product_listing:
                ...
                # avoid adding products with words like "Brush", "Collection", "Primer", "Concealer", "Serum", "Set", "Conditioner", "Highlighter", "Moisturizer", "Gel" in their name
                if(avoid_product_types(product["currentSku"]["imageAltText"])):
...
def main():
    ...
    total_results = int(get_total_results(session, "foundation", 1))
    ...
    while results_scraped <= total_results:
        ...
        search_results = search_api(session, "foundation", currIndex)
    ...
    # create a csv file
    with open('sephora_foundation.csv', 'w', newline='') as csvfile:
